    ...
    <>
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-gradient-to-br from-orange-950/60 to-zinc-900 border border-orange-500/20 rounded-2xl p-5">
          ...
          <button
            ...
            disabled={summary.confirmed < MIN_WITHDRAWAL}
            className="mt-4 w-full py-3 rounded-xl font-bold text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            style={{
              background: summary.confirmed >= MIN_WITHDRAWAL
                ? 'linear-gradient(135deg, #f97316, #ea580c)'
                : '#1f1f1f',
              color: summary.confirmed >= MIN_WITHDRAWAL ? '#000' : '#555',
            }}
          >
            {summary.confirmed >= MIN_WITHDRAWAL
              ? `Withdraw ₹${summary.confirmed.toFixed(0)}`
              : `Earn ₹${(MIN_WITHDRAWAL - summary.confirmed).toFixed(0)} more to withdraw`}
          </button>
        </div>
      ...
      {userId && (
        <WithdrawModal
          ...
          confirmedBalance={summary.confirmed}
          userId={userId}
