...
export default function AdminPage() {
  ...
  const openEdit = (coupon: Coupon) => {
    ...
    setForm({ brand_name: coupon.brand_name, coupon_code: coupon.coupon_code, discount_description: coupon.discount_description, category: coupon.category, expiry_date: coupon.expiry_date?.split("T")[0] || "", affiliate_link: coupon.affiliate_link || "", conditions: coupon.conditions || "" })
    setShowModal(true)
  }
  ...
  const categoryCounts = CATEGORIES.map(cat => ({ name: cat, count: coupons.filter(c => c.category === cat).length })).filter(c => c.count > 0)
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="border-b border-border bg-secondary/30 px-6 py-4">
        <h1 className="text-2xl font-bold text-foreground">PrimeSavr <span className="text-primary">Admin</span></h1>
        ...
        <div className="flex gap-1">
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key as Tab)}
              className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === "coupons" && (
          <div>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={openAdd} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90">+ Add Coupon</button>
            </div>
            <div className="overflow-x-auto rounded-lg border border-border">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50">
                  <tr>{["Brand", "Code", "Category", "Description", "Expiry", "Status", "Upvotes", "Actions"].map(h => <th key={h} className="px-4 py-3 text-left font-medium text-muted-foreground">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {coupons.map(c => (
                    <tr key={c.id} className="border-t border-border hover:bg-secondary/20">
                      <td className="px-4 py-3 font-medium">{c.brand_name}</td>
                      <td className="px-4 py-3"><code className="rounded bg-secondary px-2 py-1 text-xs">{c.coupon_code}</code></td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`rounded-full px-2 py-1 text-xs font-medium ${c.status === "approved" ? "bg-green-500/20 text-green-400" : c.status === "rejected" ? "bg-red-500/20 text-red-400" : "bg-yellow-500/20 text-yellow-400"}`}>{c.status}</span>
                      </td>
              ...
              {coupons.length === 0 && <p className="py-8 text-center text-muted-foreground">No coupons yet.</p>}
            </div>
          </div>
        )}
        ...
        {activeTab === "approvals" && (
          <div>
            ...
            <div className="overflow-x-auto rounded-lg border border-border">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50">
                  <tr>{["Brand", "Code", "Category", "Description", "Actions"].map(h => <th key={h} className="px-4 py-3 text-left font-medium text-muted-foreground">{h}</th>)}</tr>
                </thead>
                <tbody>
              ...
              {pendingCount === 0 && <p className="py-8 text-center text-muted-foreground">No pending approvals 🎉</p>}
            </div>
          </div>
        )}
        ...
        {activeTab === "users" && (
          <div>
            ...
            <div className="overflow-x-auto rounded-lg border border-border">
              <table className="w-full text-sm">
                <thead className="bg-secondary/50">
                  <tr>{["Email", "Admin", "Joined", "Actions"].map(h => <th key={h} className="px-4 py-3 text-left font-medium text-muted-foreground">{h}</th>)}</tr>
                </thead>
                <tbody>
                  {users.map(u => (
                    <tr key={u.id} className="border-t border-border hover:bg-secondary/20">
                      ...
                      <td className="px-4 py-3">
                        <span className={`rounded-full px-2 py-1 text-xs font-medium ${u.is_admin ? "bg-primary/20 text-primary" : "bg-secondary text-muted-foreground"}`}>{u.is_admin ? "Admin" : "User"}</span>
                      </td>
                      <td className="px-4 py-3">{new Date(u.created_at).toLocaleDateString()}</td>
                      <td className="px-4 py-3">
                        <button onClick={() => toggleAdmin(u.id, u.is_admin)} className="rounded bg-secondary px-2 py-1 text-xs hover:bg-secondary/80">{u.is_admin ? "Revoke Admin" : "Make Admin"}</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        {activeTab === "analytics" && (
          <div>
            ...
            <div className="rounded-lg border border-border bg-secondary/30 p-6">
              ...
              {categoryCounts.length === 0 ? <p className="text-muted-foreground">No data yet.</p> : (
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-lg rounded-xl border border-border bg-background p-6 shadow-xl">
            <h3 className="mb-4 text-lg font-semibold">{editingCoupon ? "Edit Coupon" : "Add New Coupon"}</h3>
            <div className="space-y-3">
              {[
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowModal(false)} className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-secondary">Cancel</button>
              <button onClick={saveCoupon} disabled={saving} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">{saving ? "Saving..." : "Save Coupon"}</button>
            </div>
          </div>
        </div>
      )}
